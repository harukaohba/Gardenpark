...
public class Jackson01_graph extends Frame implements ActionListener,WindowListener{
	...
	public Jackson01_graph(ArrayList<Integer>[] arrayLists, int N, int[] node_index) {
		...
		this.queuelength = arrayLists;
		DefaultCategoryDataset data = new DefaultCategoryDataset();
		int sum;
		for(int i = 0; i < 1000; i++) {
			sum = 0;
			for(int j = 0; j < N; j++) {
				data.addValue(this.queuelength[j].get(i),node_index[j]+"", i+"");
