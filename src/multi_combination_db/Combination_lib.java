...
public class Combination_lib {
	...
	public Combination_lib(int n, int c) {
		...
		// combi = this.factorial(n + c - 1) / (this.factorial(n) * this.factorial(c -
		// 1));
	...
	public int[][] getValueNc(int value[][], int maxclassnum) {
		int nc_number = 0;
		// 条件に当てはまる組合せを数える
		for (int i = 0; i < value.length; i++) {
			int flg = 1;
			for (int j = 0; j < c; j++) {
				if (value[i][j] > maxclassnum)
					flg = -1;
			}
			if (flg == 1)
				nc_number++;
	...
	/*
	 * ループでやる時 public int[][] getCombination() { int value[][] = new int[6][c]; int
	 * index = 0; for(int i = 0; i <= n; i++) {//第１層 value[index][0] = i; for(int j
	 * = 0; j <= n-i;j++) {//第２層 value[index][1] = j; //最下層 value[index][2] = n - i
	 * - j; index++; } } return value; }
	 */
